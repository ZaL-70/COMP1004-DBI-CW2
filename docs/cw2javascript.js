...
async function updateVehicleResults() {
     ...
     const driverNameElement = document.getElementById("txtRegNum"); // Get user input
     driverName = driverNameElement.value.trim().toLowerCase();
     ...
     // Check if the input is a substring of any name
     for (const pQuery of arrQuery) {
          let vRegNum = pQuery.VehicleID.toLowerCase();
          if (vRegNum.includes(regNum) && regNum !== "") {
               found = true;
               const { data: arrVehicles, error: allSelError } = await supabase.from("Vehicles").select().eq("Vehicle", pQuery.VehicleID);
               const results = document.createElement("ul");
